...
export const guildMemberAdd = client.on('guildMemberAdd', async (newMember) => {
    ...
    const bem = channelItsGuildTextChannel(newMember.guild.channels.cache.find((channel) => channel.name.includes("bem-vindo")
    ));
    const regras = channelItsGuildTextChannel(newMember.guild.channels.cache.find((channel) => channel.name.includes("regras"))
    );
    const cargos = channelItsGuildTextChannel(newMember.guild.channels.cache.find((channel) => channel.name.includes("cargos"))
    );
