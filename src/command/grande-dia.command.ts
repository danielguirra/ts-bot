...
export class GrandediaCommand extends SlashCommandBuilder {
   constructor() {
      super();
      this.setName('grandedia')
   ...
   async executeMessageCommand(commandMessage: Message) {
      ...
      if (user && channel) {
         const image = await this.createCanvasGrandeDia(user);
         if (image) {
   ...
   async executeSlashCommand(commandSlash: CommandInteraction) {
      ...
      if (user && channel) {
         return loadinCreator(
            commandSlash,
            async () => {
               return await this.createCanvasGrandeDia(user);
            },
            undefined
         );
      }
   ...
   async createCanvasGrandeDia(user: User) {
