...
const sendClimateCurrentTime = async (channel, city, channelSlash) => {
    ...
    try {
        ...
        if (channelSlash) {
            return (0, getEmbed_1.embedBuilder)(`Clima de ${cityNameToReturnInEmbed} agora ${str_hora}`, ` A temperatura está por volta de :**${climate.temp_C}Cº**
        ...
        if (channel)
            try {
                channel.send({
                    embeds: [
                        (0, getEmbed_1.embedBuilder)(`Clima de ${cityNameToReturnInEmbed} agora ${str_hora}`, ` A temperatura está em :**${climate.temp_C}Cº**
          Humidade em **${climate.humidity}%**
          **${climate.text}** ${climate.emoji}
          Sensação térmica de **${climate.heatIndex}Cº**
        `),
